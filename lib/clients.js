...
// Native Streamable HTTP entry formats — no mcp-remote proxy needed.
// Each client has its own field conventions for remote HTTP servers.
function makeClaudeEntry(url) { return { type: 'http', url }; }
...
function preview({ force = false, includeAll = false, customPaths = [] } = {}) {
  ...
  for (const rawPath of customPaths) {
    ...
    // Custom paths don't have an associated client — default to Claude HTTP format.
...
module.exports = {
  ...
  makeClaudeEntry, makeCursorEntry, makeWindsurfEntry, resolveUrl,
